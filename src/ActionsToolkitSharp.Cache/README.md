# ActionsToolkitSharp.Cache

[![NuGet](https://img.shields.io/nuget/v/ActionsToolkitSharp.Cache?label=ActionsToolkitSharp.Cache&logo=nuget)](https://www.nuget.org/packages/ActionsToolkitSharp.Cache)
...
```xml
<PackageReference Include="ActionsToolkitSharp.Cache" Version="[Version]" />
```
...
| Name | Purpose |
| --- | --- |
| `ActionsToolkitSharp.Cache` | Twirp transport (bearer auth from `ACTIONS_RUNTIME_TOKEN`). |
| `ActionsToolkitSharp.Cache.Blob` | Bare client for blob upload/download via signed URLs (no auth). |
