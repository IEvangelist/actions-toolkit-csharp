...
internal sealed class DefaultArtifactClient : IArtifactClient
{
    ...
    /// </summary>
    internal const string BlobHttpClientName = "ActionsToolkit.Artifact.Blob";
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var stagingPath = Path.Combine(Path.GetTempPath(), $"actions-toolkit-artifact-dl-{Guid.NewGuid():N}.zip");
            string digest;
            try
            {
