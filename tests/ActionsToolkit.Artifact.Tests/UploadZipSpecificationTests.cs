...
public sealed class UploadZipSpecificationTests : IDisposable
{
    ...
    {
        _root = Path.Combine(Path.GetTempPath(), "actions-toolkit-upload-zip-spec-" + Guid.NewGuid().ToString("N"));
    ...
    {
        var outside = Path.Combine(Path.GetTempPath(), "actions-toolkit-upload-zip-spec-outside-" + Guid.NewGuid().ToString("N") + ".txt");
