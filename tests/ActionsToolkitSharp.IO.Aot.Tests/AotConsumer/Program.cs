...
namespace ActionsToolkitSharp.IO.AotConsumer;

/// <summary>
/// Native AOT dispatcher that exercises the public surface of
/// <see cref="ActionsToolkitSharp.IO"/> so that the trimmer roots every API.
...
internal static class Program
{
    private static int Main(string[] args)
    {
        if (args.Length == 0)
        {
            return Fail("Usage: ats-io-aot-consumer <case> [sandbox-path]");
        }
        ...
        try
        {
            ...
            var sandbox = args.Length >= 2 ? args[1] : Path.Combine(Path.GetTempPath(), "ats-io-aot-" + Guid.NewGuid().ToString("N"));
