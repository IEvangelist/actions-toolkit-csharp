...
using ActionsToolkitSharp.Cache.Errors;
using ActionsToolkitSharp.Cache.Internal;
using ActionsToolkitSharp.Cache.Internal.Twirp;
using ActionsToolkitSharp.Cache.Json;
using ActionsToolkitSharp.Cache.Models;
using ActionsToolkitSharp.Cache.Services;

using Microsoft.Extensions.DependencyInjection;

namespace ActionsToolkitSharp.Cache.AotConsumer;

/// <summary>
/// Native AOT dispatcher that exercises the public surface of
/// <c>ActionsToolkitSharp.Cache</c>. Mirrors the save / restore / lookup
...
internal static class Program
{
    private static int Main(string[] args)
    {
        if (args.Length == 0)
        {
            return Fail("Usage: ats-cache-aot-consumer <case>");
        }
    ...
    {
        var work = Path.Combine(Path.GetTempPath(), "ats-cache-aot-" + Guid.NewGuid().ToString("N"));
