...
using ActionsToolkit.Attest.Auth;
using ActionsToolkit.Attest.Json;
using ActionsToolkit.Attest.Models;
using ActionsToolkit.Attest.Services;
using ActionsToolkit.Attest.Signing;
using ActionsToolkit.Attest.Store;
...
namespace ActionsToolkit.Attest.AotConsumer;

/// <summary>
/// Native AOT dispatcher exercising the public surface of
/// <see cref="ActionsToolkit.Attest"/> so the trimmer roots every API
...
internal static class Program
{
    private static int Main(string[] args)
    {
        if (args.Length == 0)
        {
            return Fail("Usage: actions-toolkit-attest-aot-consumer <case>");
        }
