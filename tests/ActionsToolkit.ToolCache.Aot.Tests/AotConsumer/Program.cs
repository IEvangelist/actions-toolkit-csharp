...
using ActionsToolkit.ToolCache.Manifest;
using ActionsToolkit.ToolCache.Semver;
using Microsoft.Extensions.DependencyInjection;
using System.Text.Json;

namespace ActionsToolkit.ToolCache.AotConsumer;

/// <summary>
/// Native AOT dispatcher that exercises the public surface of
/// <see cref="ActionsToolkit.ToolCache"/> so that the trimmer roots every
...
internal static class Program
{
    private static int Main(string[] args)
    {
        if (args.Length == 0)
        {
            return Fail("Usage: actions-toolkit-toolcache-aot-consumer <case> [sandbox-path]");
        }
        ...
        try
        {
            var sandbox = args.Length >= 2
                ? args[1]
                : Path.Combine(Path.GetTempPath(), "actions-toolkit-tc-aot-" + Guid.NewGuid().ToString("N"));
