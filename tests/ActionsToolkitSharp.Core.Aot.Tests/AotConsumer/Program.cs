...
namespace ActionsToolkitSharp.Core.AotConsumer;

/// <summary>
/// Native AOT dispatcher that exercises the public surface of
/// <see cref="ActionsToolkitSharp.Core"/>. Each <c>case</c> mirrors a concern from
...
internal static class Program
{
    private static int Main(string[] args)
    {
        if (args.Length == 0)
        {
            return Fail("Usage: ats-core-aot-consumer <case>");
        }

        var sandbox = Path.Combine(Path.GetTempPath(), "ats-core-aot-" + Guid.NewGuid().ToString("N"));
