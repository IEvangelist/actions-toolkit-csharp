...
namespace ActionsToolkitSharp.Glob.AotConsumer;

/// <summary>
/// Native AOT dispatcher that exercises the public surface of
/// <see cref="ActionsToolkitSharp.Glob"/>. Mirrors concerns from the upstream
...
internal static class Program
{
    private static int Main(string[] args)
    {
        if (args.Length == 0)
        {
            return Fail("Usage: ats-glob-aot-consumer <case> [sandbox-path]");
        }
    ...
    {
        var sandbox = Path.Combine(Path.GetTempPath(), "ats-glob-aot-" + Guid.NewGuid().ToString("N"));
